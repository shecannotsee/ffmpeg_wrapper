...
void pgm_save(av_frame& frame, const std::string& file_name) {
  auto buf  = frame.get()->data[0];
  int wrap  = frame.get()->linesize[0];
  int xsize = frame.get()->width;
  int ysize = frame.get()->height;
...
void jpeg_save(av_frame& frame, const std::string& file_name) {
  auto format = (enum AVPixelFormat)frame.get()->format;
  if (format != AV_PIX_FMT_YUV420P      // 标准的 YUV 420 格式
  ...
  auto jpeg_ctx = ctx.get_jpeg_encode(frame.get()->width, frame.get()->height);
