#include <she_test.h>
#include <test_support.h>

#include <gsl/gsl>

extern "C" {
#include <libavcodec/avcodec.h>
}

namespace {
constexpr int INBUF_SIZE = 4096;

void pgm_save(unsigned char* buf, int wrap, int xsize, int ysize, const std::string& filename) {
  FILE* f;
  int i;

  f = fopen(filename.c_str(), "w");
  fprintf(f, "P5\n%d %d\n%d\n", xsize, ysize, 255);
  for (i = 0; i < ysize; i++) {
    fwrite(buf + i * wrap, 1, xsize, f);
  }
  fclose(f);
}

void decode(AVCodecContext* dec_ctx, AVFrame* frame, AVPacket* pkt, const std::string& filename) {
  char buf[1024];
  int ret;

  ret = avcodec_send_packet(dec_ctx, pkt);
  if (ret < 0) {
    fprintf(stderr, "Error sending a packet for decoding\n");
    exit(1);
  }

  while (ret >= 0) {
    ret = avcodec_receive_frame(dec_ctx, frame);
    if (ret == AVERROR(EAGAIN) || ret == AVERROR_EOF)
      return;
    else if (ret < 0) {
      fprintf(stderr, "Error during decoding\n");
      exit(1);
    }

    printf("saving frame %3d\n", dec_ctx->frame_number);
    fflush(stdout);

    /* the picture is allocated by the decoder. no need to
       free it */
    snprintf(buf, sizeof(buf), "%s-%d", filename.c_str(), dec_ctx->frame_number);
    pgm_save(frame->data[0], frame->linesize[0], frame->width, frame->height, buf);
  }
}
}  // namespace

SHE_TEST(ffmpeg_4_1, sample_decode_video) {
  const std::string filename   = "../resource/KTMR2R.h264";
  const std::string output_dir = "./decode_video";
  test_support::create_dir(output_dir);
  const std::string outfilename = output_dir + "/decode";

  /* 将缓冲区结束设置为 0（这可确保损坏的 MPEG 流不会发生过度读取） */
  uint8_t inbuf[INBUF_SIZE + AV_INPUT_BUFFER_PADDING_SIZE];
  memset(inbuf + INBUF_SIZE, 0, AV_INPUT_BUFFER_PADDING_SIZE);

  /* find the MPEG-1 video decoder */
  const AVCodec* codec = avcodec_find_decoder(AV_CODEC_ID_H264);
  if (!codec) {
    fprintf(stderr, "Codec not found\n");
    exit(1);
  }

  AVCodecParserContext* parser = av_parser_init(codec->id);
  auto parser_release          = gsl::finally([&]() { av_parser_close(parser); });
  /* check */ {
    if (!parser) {
      fprintf(stderr, "parser not found\n");
      exit(1);
    }
  }

  AVCodecContext* c = avcodec_alloc_context3(codec);
  auto c_release    = gsl::finally([&]() { avcodec_free_context(&c); });
  /* check */ {
    if (!c) {
      fprintf(stderr, "Could not allocate video codec context\n");
      exit(1);
    }
  }

  /* 对于某些编解码器（如 msmpeg4 和 mpeg4），必须在那里初始化宽度和高度，因为此信息在比特流中不可用。 */

  /* open it */
  if (avcodec_open2(c, codec, NULL) < 0) {
    fprintf(stderr, "Could not open codec\n");
    exit(1);
  }

  FILE* f        = fopen(filename.c_str(), "rb");
  auto f_release = gsl::finally([&]() { fclose(f); });
  /* check */ {
    if (!f) {
      fprintf(stderr, "Could not open %s\n", filename.c_str());
      exit(1);
    }
  }

  AVFrame* frame     = av_frame_alloc();
  auto frame_release = gsl::finally([&]() { av_frame_free(&frame); });
  /* check */ {
    if (!frame) {
      fprintf(stderr, "Could not allocate video frame\n");
      exit(1);
    }
  }

  AVPacket* pkt    = av_packet_alloc();
  auto pkt_release = gsl::finally([&]() { av_packet_free(&pkt); });
  if (!pkt) exit(1);

  while (!feof(f)) {
    /* 从输入文件中读取原始数据 */
    size_t data_size = fread(inbuf, 1, INBUF_SIZE, f);
    if (!data_size) break;

    /* 使用解析器将数据拆分为帧 */
    const uint8_t* data = inbuf;
    while (data_size > 0) {
      const int ret =
          av_parser_parse2(parser, c, &pkt->data, &pkt->size, data, data_size, AV_NOPTS_VALUE, AV_NOPTS_VALUE, 0);
      if (ret < 0) {
        fprintf(stderr, "Error while parsing\n");
        exit(1);
      }
      data += ret;
      data_size -= ret;

      if (pkt->size) decode(c, frame, pkt, outfilename);
    }
  }

  /* flush the decoder */
  decode(c, frame, NULL, outfilename);

  // fclose(f);
  // av_parser_close(parser);
  // avcodec_free_context(&c);
  // av_frame_free(&frame);
  // av_packet_free(&pkt);
  return true;
}